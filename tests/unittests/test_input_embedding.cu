...
int main(int argc, char *argv[]) {
    ...
    if (argv[1]) {
        ...
        EmbeddingWeight<float>* emb_table = new EmbeddingWeight<float>();
        emb_table -> data = d_table;
        // EmbeddingWeight<float>* emb_ptr = &emb_table;
        launchEmbeddingFactor(input_ids, emb_table, output); // & 相当于传指针吗？那么这里为什么不new一个呢
        ...
        EmbeddingWeight<half>* emb_table = new EmbeddingWeight<half>();
        emb_table -> data = d_table;
        // EmbeddingWeight<float>* emb_ptr = &emb_table;
        launchEmbeddingFactor(input_ids, emb_table, output); // & 相当于传指针吗？那么这里为什么不new一个呢
